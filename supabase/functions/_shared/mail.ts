...
export async function sendOtpEmail({ to, otp }: SendOtpParams): Promise<void> {
  ...
  const from =
    Deno.env.get('RESEND_FROM_EMAIL')?.trim() || 'ExpTrack <onboarding@resend.dev>';
  ...
  const subject = 'Your ExpTrack password reset code';
...
<p style="color:#334155;line-height:1.5">Use this 6-digit code in the ExpTrack app. It expires in ${OTP_TTL_MINUTES} minutes.</p>
...
<p style="color:#64748b;font-size:13px">If you did not request this, you can ignore this email.</p>
  ...
  const text = `Your ExpTrack password reset code is ${otp}. It expires in ${OTP_TTL_MINUTES} minutes.`;
